...
def GenMass(key = 1):
    ...
    #те вершины куда ты можешь попасть (делай минус 1(пример попадаешь в 2 и 4 - пишешь 1 и 3))
    arr = [1, 3]
    ...
    if key == 1:
        for i in range(len(array_row)):
            for j in range(i, len(array_row)):
                if i == 0 and j in arr:
                    array_row[i][j] = random.randint(1, 9)
                    array_row[j][i] = array_row[i][j]
                if i in [1, 2, 3] and j in [1, 2, 3]:
                    array_row[i][j] = random.randint(1, 9)
                    array_row[j][i] = array_row[i][j]
                if i in [3, 4, 5, 6] and j in [3, 4, 5, 6]:
                    array_row[i][j] = random.randint(1, 9)
                    array_row[j][i] = array_row[i][j]
                if i in [6, 7, 8, 9] and j in [6, 7, 8, 9]:
                    array_row[i][j] = random.randint(1, 9)
                    array_row[j][i] = array_row[i][j]
                if i in [9, 10] and j in [9, 10]:
                    array_row[i][j] = random.randint(1, 9)
                    array_row[j][i] = array_row[i][j]
                if i == j:
                    array_row[i][j] = 0
    else:
        width = int(input("Введите ширину ленты: "))
        for i in range(len(array_row)):
            for j in range(i, len(array_row)):
                if i == 0 and j in arr:
                    array_row[i][j] = random.randint(1, 9)
...
def FindAllPaths():
    new_arr = GenMass(2) # GenMass(key=1) при key = 1 -- блочно-диагональная, при key = 2 -- ленточная
    d = {}
...
def Algorithm():
    ...
    stop = 10  # делай - 1, если из 5ки в 9ку, то стоп равен 8
    way = [stop]
    length = d_p[0][stop]
